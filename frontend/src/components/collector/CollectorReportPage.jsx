...
export default function CollectorHomePage() {
  ...
  const fetchCollections = async () => {
    try {
      ...
      const calculatedWeight = response.data.reduce((acc, collection) => acc + collection.weight, 0);
      setTotalWeight(calculatedWeight); // Update totalWeight state
    } catch (error) {
      console.log(error);
    }
  };
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="bg-[#2F5944] p-6 relative h-[250px]">
        <div className="flex justify-between mt-3 ml-[100px]">
          <div>
            <h1 className="text-white text-lg -mt-2 font-semibold">Collection Report</h1>
          </div>
        </div>
        <button className="mt-2" onClick={() => navigate('/collectormappage')}><IoMdArrowRoundBack className="text-white size-7" /></button>
        <p className="float-right my-2 bg-emerald-950 text-white px-6 py-0.5 rounded-lg">
        ...
        {users.length > 0 && (
          <div className="mt-2 relative w-[80%] mx-auto text-white">
            <p className="font-bold">{users[2].address}</p>
            <div className="flex justify-between">
              <p className="font-semibold">{users[2].name}</p>
              <p>{users[2].phoneNumber}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-t-[35px] z-10 -mt-7">
        ...
        <div className="flex justify-around -mt-12 mb-0 px-4 py-6">
          <div className="bg-[#1E441F] p-4 rounded-lg w-[85%] text-center inline-flex">
            <div className="flex justify-between w-full">
              <p className="text-md font-semibold text-white">Total Waste Collected</p>
              <p className="text-lg font-semibold text-white">{totalWeight} kg</p> {/* Display total weight */}
            </div>
          </div>
        </div>
        ...
        <div className="bg-transparent mx-4 p-2 rounded-xl flex flex-col items-center relative">
          {collections.map((collection, index) => {
            console.log(collection); // Log to check the data
            return ( // Add return statement to render JSX
              <div key={index} className="mt-3 ml-0 w-[100%] bg-white p-2 flex space-x-6 border-b-2 rounded-md">
                <img src={cRep1} alt="Collection representation" />
                <div className="flex justify-between space-x-16 pr-4 pt-2 w-full">
                  <h1 className="font-semibold text-lg -mt-2">{collection.wasteType}</h1>
                  <h1 className="font-bold text-xl text-right -mt-2">{collection.weight} kg</h1>
                </div>
              </div>
            );
          })}
      ...
      <div className="mt-auto">
        <button
          onClick={() => navigate("/collectorsuccessfulpage")}
          className="w-[90%] ml-5 h-16 font-semibold text-lg px-5 py-2 rounded-full bg-[#2F5944] text-white hover:bg-[#214131] transition-all mb-2 mt-7"
        >
          Submit
        </button>
